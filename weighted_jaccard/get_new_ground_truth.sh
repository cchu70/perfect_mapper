...
# filter the samtools alignment to rempve split and only consider primary reads. This is so each read gets only one alignment
samtools -F 2048 view chrX.prepolished.orig_reads.minimap2.N50_r3k.sam | awk '$2 == 0 {print $1"\t"$4}' > chrX.prepolished.orig_reads.ground_truth_pos.txt



# script to compare the simulated alignments to this ground truth
awk '{ if(NR == FNR) {keys[$1] = $2 } else{ } }'
