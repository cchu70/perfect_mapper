...
def main():
	...
	for file in open(fofn, "r"):
		...
		for line in open(file.strip(), "r"):
			...
			# from_A_unaligned = int(data[6]) # Include in future analysis
			...
			to_print_A = "%0.8f\t%0.8f\tGAGE_%s\t%s" % (err, A_correct, which_err, "GAGE_A")
			to_print_B = "%0.8f\t%0.8f\tGAGE_%s\t%s" % (err, B_correct,  which_err, "GAGE_B")
			print(to_print_A)
			print(to_print_B)
